...
void execute(Seq_T segment_0)
{
       ...
       // Seq_T registers = createRegisters();
        ...
        bool executing = true;
        loadInstruction loadInfo = newLoad();
        generalInstruction genInfo = newGeneral();
        ...
        while (executing) {
                ...
                if (opcode == 13) {
                        unpackLoad(instruction, loadInfo);
                } else {
                        unpackGeneral(instruction, genInfo);
                }
 ...
 ***********************************************************************/
void unpackLoad(uint32_t instruction, loadInstruction newInstruction) 
{
 ...
 *  Expects: instruction must be valid and packed in the correct manner
 ***********************************************************************/
void unpackGeneral(uint32_t instruction, generalInstruction newInstruction)
{
